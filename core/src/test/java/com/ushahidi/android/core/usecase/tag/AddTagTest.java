...
/**
 * Test AddTag use case
 *
 * @author Ushahidi Team <team@ushahidi.com>
 */
public class AddTagTest {

    private AddTag mAddTag;
    ...
    public void setUp() {
        ...
        mAddTag = new AddTag(mMockTagRepository, mMockThreadExecutor,
                mMockPostExecutionThread);

    }

    @Test
    public void testAddTagExecution() {
        doNothing().when(mMockThreadExecutor).execute(any(IInteractor.class));

        AddTag.Callback mockAddTagCallback = mock(AddTag.Callback.class);

        mAddTag.execute(mMockTag, mockAddTagCallback);
    ...
    @Test
    public void testAddTagRun() {
        doNothing().when(mMockThreadExecutor).execute(any(IInteractor.class));
        AddTag.Callback mockAddTagCallback = mock(AddTag.Callback.class);
        doNothing().when(mMockTagRepository).addTag(any(Tag.class),
                any(ITagRepository.TagAddCallback.class));

        mAddTag.execute(mMockTag, mockAddTagCallback);
        mAddTag.run();

        verify(mMockTagRepository).addTag(any(Tag.class),
                any(ITagRepository.TagAddCallback.class));
    ...
    public void testAddTagNullParameter() {
        mAddTag.execute(mMockTag, null);
    }
}
